...
tree Search_and_Destroy(element e, tree t)
{
	...
	tree pl = emptyTree();		tree pr = emptyTree();		/*Puntatori al padre di t, previous left & previous right;*/
															/*pl significa che il padre del nodo da eliminare lo ha come figlio sinistro, di conseguenza pr � emptyTree();*/

	while (Empty(t) == false && IsEqual(e, headTree(t)) == false) /*Cerco il nodo da eliminare ricordando di chi � figlio;*/
	{
	...
	bool IsRoot = false;									/*Mi dice se il nodo da eliminare � la radice dell'albero;*/
	if (Empty(pl) == true && Empty(pr) == true)
		IsRoot = true;

	/*Il nodo da eliminare pu� essere una foglia, oppure avere 1 o 2 figli, 3 casi in tutto:*/

	/*Il nodo � una foglia: occorre solo eliminare il nodo;*/
	...
	if (Empty(next) == false)
	{
		...
		else														/*Altrimenti, il nodo da eliminare pu� essere il figlio destro o la radice dell'albero;*/
	...
	{
		tree x = right(t);						/*x � il puntatore al nodo succ() di t;*/
		tree pl_x = x;							/*Puntatore al padre di x, se x � la radice pl_x = x il succ() di t;*/
...
tree2 Search_and_Destroy2(element2 e, tree2 t)
{
	...
	tree2 pl = emptyTree2();		tree2 pr = emptyTree2();		/*Puntatori al padre di t, previous left2 & previous right2;*/
															/*pl significa che il padre del nodo da eliminare lo ha come figlio sinistro, di conseguenza pr � emptyTree2();*/

	while (Empty2(t) == false && IsEqual2(e, headTree2(t)) == false) /*Cerco il nodo da eliminare ricordando di chi � figlio;*/
	{
	...
	{
		tree2 x = right2(t);						/*x � il puntatore al nodo succ2() di t;*/
		tree2 pl_x = x;							/*Puntatore al padre di x, se x � la radice pl_x = x il succ2() di t;*/
